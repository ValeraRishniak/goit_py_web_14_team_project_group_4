...
async def update_avatar_user(file: UploadFile = File(), current_user: User = Depends(auth_service.get_current_user),
    ...
    r = cloudinary.uploader.upload(
        file.file, public_id=f'PhotoSHAKE/{current_user.username}/{current_user.id}', overwrite=True)
    src_url = cloudinary.CloudinaryImage(f'PhotoSHAKE/{current_user.username}/{current_user.id}')\
