...
async def create_tag(tags: List[ImageTagModel], current_user: User = Depends(auth_service.get_current_user), db: Session = Depends(get_db)):
...
async def remove_tag(tag_id: int, current_user: User = Depends(auth_service.get_current_user), db: Session = Depends(get_db)):
