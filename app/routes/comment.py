...
@router.post(
    "/", response_model=List[CommentResponse], status_code=status.HTTP_201_CREATED
)
...
@router.delete(
    "/delete/{comment_id}",
    response_model=CommentResponse,  # dependencies=[Depends(allowed_remove_comments)]
)
...
@router.get(
    ...
    response_model=CommentResponse,  # dependencies=[Depends(allowed_get_comments)]
)
