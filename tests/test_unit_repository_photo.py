...
from app.repository.photo import get_photos,   get_photo,  add_photo , update_description , remove_photo
...
class TestContact(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_photos(self):
        ...
        self.session.query().offset().limit().filter().all.return_value = photo
    ...
    async def test_get_photo(self):
        ...
        result = await get_photo(photo_id=photo_id , user=self.user, db=self.session)
        self.assertEqual(result,  photka )

    async def test_if_get_photo_not_found(self):
